...
'일단 코드 작성한 다음에 성공하면 서보모터 값 조정 스텝들을 함수화 해서 작성할까?? -> 그러면 자동으로 연속걸음이 됨 왜지
...
'왼발 들기
	...
	GOTO 전진걸음2 '다음 걸음 스텝으로 가기

    ELSE '보행순서=1 인 경우, (한 걸음 걷고 그 다음 또 걸을 경우?)
	보행순서 = 0
'왼쪽으로 기울기
	SPEED 4
        MOVE G6D,  88,  74, 144,  95, 110
...
전진걸음2:
    ETX 4800,11 '진행코드를 보냄 <-?? 이 부분 이해 못했음 어디서 받게되는데???
'왼발 뻗기
    SPEED 보행속도 'speed 13 (전진걸음1에서 할당됨)
    MOVE G6A, 86,  56, 145, 115, 110
    MOVE G6D,108,  76, 147,  93,  96
    WAIT
